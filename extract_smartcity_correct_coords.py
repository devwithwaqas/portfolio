#!/usr/bin/env python3
"""
Extract CORRECT Smart City C4 Diagram Coordinates
Properly analyze the actual SVG structure and extract real component positions
"""
...
def extract_correct_smartcity_coordinates(svg_file: str) -> Dict[str, Any]:
    """Extract correct coordinates from Smart City C4 diagram"""
    ...
    for group in all_groups:
        ...
        print(f"Processing group: {group_id} (class: {group_class})")
        ...
        for rect in rectangles:
            ...
            # More lenient area filtering - focus on meaningful components
            if 500 < area < 1000000:  # Reasonable component size range
                ...
                if rect_key not in processed_rects:
                    ...
                    print(f"  Found rect: x={x:.1f}, y={y:.1f}, w={width:.1f}, h={height:.1f}, area={area:.0f}")
        ...
        for ellipse in ellipses:
            ...
            if 500 < area < 1000000:
                ...
                if rect_key not in processed_rects:
                    ...
                    processed_rects.add(rect_key)
                    print(f"  Found ellipse: x={x:.1f}, y={y:.1f}, w={width:.1f}, h={height:.1f}, area={area:.0f}")
                ...
                if len(coords) >= 6:  # At least 3 points
                    ...
                    if 500 < area < 1000000:
                        ...
                        if rect_key not in processed_rects:
                            ...
                            processed_rects.add(rect_key)
                            print(f"  Found polygon: x={min_x:.1f}, y={min_y:.1f}, w={width:.1f}, h={height:.1f}, area={area:.0f}")
        ...
        if is_users and valid_rects:
            ...
            print(f"  Created group: x={group_component['x']:.1f}, y={group_component['y']:.1f}, w={group_component['width']:.1f}, h={group_component['height']:.1f}")
            ...
            for i, rect_data in enumerate(valid_rects):
                ...
                print(f"  Added individual: {component_text} - x={rect_data['x']:.1f}, y={rect_data['y']:.1f}, w={rect_data['width']:.1f}, h={rect_data['height']:.1f}")
...
def main():
    ...
    print("Extracting CORRECT Smart City C4 diagram coordinates...")
    components = extract_correct_smartcity_coordinates(svg_file)
    ...
    # Save to JSON
    output_file = "smartcity_correct_coordinates.json"
