...
class ArchitectureDiagram {
  ...
  createControls() {
    ...
    const existingControls = this.container.querySelector('.diagram-controls');
    if (existingControls) {
    ...
    controls.innerHTML = `
      <button class="diagram-control-btn" title="Zoom In">
        <i class="bi bi-zoom-in"></i> Zoom In
      </button>
      <button class="diagram-control-btn" title="Zoom Out">
        <i class="bi bi-zoom-out"></i> Zoom Out
      </button>
      <button class="diagram-control-btn" title="Reset View">
        <i class="bi bi-arrows-fullscreen"></i> Reset
      </button>
      <button class="diagram-control-btn" title="Fit to View">
        <i class="bi bi-aspect-ratio"></i> Fit
      </button>
    `;
    
    // Bind button events
    const buttons = controls.querySelectorAll('.diagram-control-btn');
    buttons[0].addEventListener('click', () => this.zoomIn());
    buttons[1].addEventListener('click', () => this.zoomOut());
    buttons[2].addEventListener('click', () => this.resetView());
    buttons[3].addEventListener('click', () => this.fitToView());
    
    // Append to the container
    this.container.appendChild(controls);
    console.log('Controls created and added to diagram');
  }
