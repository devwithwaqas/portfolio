(function(){
  function initDiagram(opts){
    ...
    const api = {
      ...
      _panZoom(){ return panZoom; }
    };
    if(buttons.zoomIn)  document.querySelector(buttons.zoomIn )?.addEventListener('click', api.zoomIn);
    if(buttons.zoomOut) document.querySelector(buttons.zoomOut)?.addEventListener('click', api.zoomOut);
    if(buttons.fit)     document.querySelector(buttons.fit    )?.addEventListener('click', api.fitToView);
    if(buttons.reset)   document.querySelector(buttons.reset  )?.addEventListener('click', api.resetView);

    const doRelayout = ()=>{ try{ panZoom.resize?.(); fitAndCenter(); }catch(e){} };
    new ResizeObserver(doRelayout).observe(stageEl);
    window.addEventListener('orientationchange', ()=> setTimeout(doRelayout, 100));
    window.addEventListener('resize', doRelayout);

    window.chartControls = api;
    return api;
  }
