...
<header id="header" class="header d-flex flex-column justify-content-center">
  ...
  <nav id="navmenu" class="navmenu">
    <ul>
      <li><a href="index.html#hero" class="nav-link"><i class="ri-home-smile-2-fill navicon"></i><span>Home</span></a></li>
      <li><a href="index.html#about" class="nav-link"><i class="ri-user-3-fill navicon"></i><span>About</span></a></li>
      <li><a href="index.html#resume" class="nav-link"><i class="ri-file-text-fill navicon"></i><span>Resume</span></a></li>
      <li><a href="index.html#portfolio" class="nav-link"><i class="ri-image-2-fill navicon"></i><span>Portfolio</span></a></li>
      <li><a href="index.html#services" class="nav-link"><i class="ri-stack-fill navicon"></i><span>Services</span></a></li>
      <li><a href="index.html#contact" class="nav-link"><i class="ri-mail-send-fill navicon"></i><span>Contact</span></a></li>
    </ul>
  </nav>
...
<div id="preloader"></div>`;
  ...
  // Load inline SVG into a target container by id
  static async loadHxDiagramSvg(targetId, callback = null){
    try{
      const target = document.getElementById(targetId);
      if(!target){ console.error(`❌ SVG target '${targetId}' not found`); return; }
      const resp = await fetch('assets/img/heat-exchanger-diagram.svg', { mode: 'cors' });
      const txt  = await resp.text();
      target.innerHTML = txt;
    }catch(err){ console.error('❌ Failed to load inline SVG:', err); }
    finally{ if (callback && typeof callback === 'function') callback(); }
  }
  ...
  static async loadNavigation() {
    return new Promise((resolve) => {
      this.loadEmbeddedTemplate('navigation', 'nav-placeholder', () => {
        ...
        // Ensure Remix Icons stylesheet is present globally
        TemplateLoader.ensureStylesheet('https://cdn.jsdelivr.net/npm/remixicon@4.3.0/fonts/remixicon.css');
  ...
  /** Ensure a stylesheet is present; inject if missing */
  ...
  static initializeMainJSFunctionality() {
    ...
    if (typeof window.initializeNavigationFunctionality === 'function') {
      window.initializeNavigationFunctionality();
      console.log('✅ Navigation functionality initialized');
    }
  }
  ...
  static updateNavigationActiveState() {
    ...
    const currentHash = currentURL.hash; // includes leading '#'
    ...
    navLinks.forEach(link => {
      ...
      const samePath = linkURL.pathname.replace(/\/+/g, '/') === currentPath;
      const sameHash = currentHash ? (linkURL.hash === currentHash) : (linkURL.hash === '' || linkURL.hash === '#hero');
      if (samePath && sameHash && !matched) {
