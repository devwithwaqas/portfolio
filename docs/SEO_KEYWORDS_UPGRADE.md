# SEO Keywords Upgrade — Content based strategy

> **Full SEO implementation:** Meta, schema, llms.txt, Speakable, sitemap, IndexNow, and file roles are documented in **docs/SEO_SINGLE_SOURCE_OF_TRUTH.md**. This doc is the **keyword data** source of truth (82 URL keyword sets).

## 1. Strategy: How Keywords Are Generated

**Do not use any fixed or generic keyword sets.** Ignore old logic (e.g. "project", "case study", "capabilities", "system design", "architecture" as filler). Every page’s keywords are derived **only from that page’s actual content**.

### Rules for Cursor / automation

1. **Read the page** (component, view, article content, project/service copy).
2. **Infer the main theme** from the content’s meaning, not from slug or metadata labels.
3. **Primary keyword (exactly 1 per page)**  
   - Long-tail, **5–8 words**.  
   - Represents the **core topic** of the page.  
   - Examples of shape (not to copy): *"heat exchanger monitoring portal petroleum operations case study"*, *"last-minute employee flight discount booking platform"*, *"clean architecture layers dependency rule .NET"*.
4. **Secondary keywords (6–8 items)**  
   - From the **actual content** only.  
   - Represent: **industry**, **problem solved**, **type of solution**, **outcome/value**.  
   - Long-tail or mid-tail as the content supports.  
   - No generic filler.
5. **Semantic keywords (6–8 items)**  
   - From the **actual content** only.  
   - For **context and LLM understanding**: architecture patterns, domain terms, methodologies, technologies (only when meaningful to the story), workflows, sub-concepts.  
   - Not for stuffing; for clarity and related concepts.
6. **Never** reuse the same keyword set across pages; each URL is **unique** and content-driven.
7. **Never** use slug or title as the only source — use the **body/content meaning**.

### Quality audit (rules vs content)

- **Primary:** Every row checked for **5–8 words**; shortened (e.g. AirAsia, REST vs GraphQL) or expanded (e.g. Home, Privacy, microservices, agile, gamification, CI/CD, event-sourcing) to fit. Content-derived, not slug/title-only.
- **Secondary / semantic:** From page content only; generic filler (e.g. standalone "architecture") removed where not content-specific.
- **Projects:** All 10 primaries include "case study"; Heat Exchanger = petroleum/oil & gas (monitoring, refineries), not financial.

### What we are not using

- Static lists (e.g. "project", "case study", "capabilities", "system design") unless the page content explicitly describes those in a meaningful way.
- Arbitrary slicing of a single list to hit a number.
- One-size-fits-all keywords across service or project pages.

### Why this approach

- Google prefers **natural, intent-driven** keywords.
- LLMs prefer **rich semantic context**.
- Each page stays **unique and meaningful**.
- **No keyword stuffing**; **scalable** for 50–500+ articles.

---

## 2. Current Code vs This Doc

The codebase today uses a **single flat meta keywords list** (up to 8 items) per page in `seo.js`. There is **no** primary/secondary/semantic split in code. This document is the **source of truth** for the **content-driven** keyword sets below. Implementation (meta tags, schema, or LLM manifest) can be added later; the tables below are **generated from page content** as per the strategy above.

---

## 3. All 81 URLs — Content-Derived Keywords

Generated by reading each page’s content (or article title + excerpt + topic + content theme). Heat exchanger and all project/service/blog keywords are derived from **actual page content** (e.g. Heat Exchanger = petroleum / oil & gas / heat exchanger monitoring / predictive maintenance — **not** financial).

---

### 3.1 Part 1 — Core pages (4 URLs, Format B)

Copy-paste safe, Cursor-friendly. Long-tail primary (5–8 words), 6–8 secondary, 6–8 semantic; content-aligned, no filler.
...
### 3.2 Part 2 — Service pages (7 URLs, Format B)

Primary 5–8 words; 6–8 secondary, 6–8 semantic; content-aligned, no filler. Tech only where the page’s meaning depends on it.

---

#### 5. Full stack development — `/services/full-stack-development`
...
#### 6. Azure cloud architecture — `/services/azure-cloud-architecture`
...
#### 7. Technical leadership — `/services/technical-leadership`
...
#### 8. Microservices architecture — `/services/microservices-architecture`
...
#### 9. Agile project management — `/services/agile-project-management`
...
#### 10. Database design & optimization — `/services/database-design-optimization`
...
#### 11. Mobile development — `/services/mobile-development`
...
### 3.3 Part 3 — Project case studies (10 URLs, Format B)

Domain + architecture + tech + outcomes; primary 5–8 words; 6–8 secondary, 6–8 semantic. Content-aligned, no stuffing. **Heat Exchanger**: petroleum / oil & gas — not financial.

---

#### 12. Heat exchanger monitoring portal — `/projects/heat-exchanger`
...
#### 13. AirAsia ID90 portal — `/projects/airasia-id90`

```json
{
  "url": "/projects/airasia-id90",
...
#### 14. BAT inhouse enterprise app — `/projects/bat-inhouse-app`
...
#### 15. PJ Smart City GIS platform — `/projects/pj-smart-city`

```json
{
  "url": "/projects/pj-smart-city",
...
#### 16. Gamified employee engagement — `/projects/gamified-employee-management`

```json
{
  "url": "/projects/gamified-employee-management",
...
#### 17. Valet parking management system — `/projects/valet-parking`

```json
{
  "url": "/projects/valet-parking",
...
#### 18. Mobile games portfolio — `/projects/mobile-games`

```json
{
  "url": "/projects/mobile-games",
...
#### 19. UK property management platform — `/projects/uk-property-management`

```json
{
  "url": "/projects/uk-property-management",
...
#### 20. G5 F&B POS system — `/projects/g5-pos`
...
#### 21. Confidential insurance clients (enterprise insurance applications) — `/projects/chubb-insurance-applications`

Page content uses **Confidential Insurance Clients** and **Top 5 Global Agency** only; no client name. Keywords aligned to visible content.

```json
{
  "url": "/projects/chubb-insurance-applications",
...
### 3.4 Blog article pages (56 URLs)

Content source: article `title`, `excerpt`, `topic`, and content theme. Primary = long-tail 5–8 words; secondary/semantic from problem, solution, and domain. **No generic filler.** Batch A (22–31) in Format B; remainder in table until Batches B–F are added.

---

#### Batch A — Articles 22–31 (Format B)

##### 22. agile-delivery-enterprise-constraints

```json
{
  "url": "agile-delivery-enterprise-constraints",
...
##### 23. ai-changing-code-review-testing

```json
{
  "url": "ai-changing-code-review-testing",
...
##### 24. ai-ides-what-they-get-right-wrong

```json
{
  "url": "ai-ides-what-they-get-right-wrong",
...
##### 25. ai-models-claude-gemini-gpt-deepseek-comparison

```json
{
  "url": "ai-models-claude-gemini-gpt-deepseek-comparison",
...
##### 26. api-gateway-vs-bff

```json
{
  "url": "api-gateway-vs-bff",
...
##### 27. azure-bicep-iac-basics

```json
{
  "url": "azure-bicep-iac-basics",
...
##### 28. azure-cloud-architecture-patterns

```json
{
  "url": "azure-cloud-architecture-patterns",
...
##### 29. azure-devops-vs-github-actions

```json
{
  "url": "azure-devops-vs-github-actions",
...
##### 30. azure-microservices-best-practices

```json
{
  "url": "azure-microservices-best-practices",
...
##### 31. azure-serverless-functions-logic-apps

```json
{
  "url": "azure-serverless-functions-logic-apps",
...
#### Batch B — Articles 32–41 (Format B)

##### 32. behavioral-design-patterns-dotnet

```json
{
  "url": "behavioral-design-patterns-dotnet",
...
##### 33. caching-strategies-redis-dotnet

```json
{
  "url": "caching-strategies-redis-dotnet",
...
##### 34. case-study-airasia-id90

```json
{
  "url": "case-study-airasia-id90",
...
##### 35. case-study-bat-inhouse-app

```json
{
  "url": "case-study-bat-inhouse-app",
...
##### 36. ci-cd-azure-devops

```json
{
  "url": "ci-cd-azure-devops",
...
##### 37. clean-architecture-dotnet

```json
{
  "url": "clean-architecture-dotnet",
...
##### 38. creational-design-patterns-dotnet

```json
{
  "url": "creational-design-patterns-dotnet",
...
##### 39. current-state-ai-coding-tools-2026

```json
{
  "url": "current-state-ai-coding-tools-2026",
...
##### 40. cursor-vs-claude-code-vs-copilot-ai-ide

```json
{
  "url": "cursor-vs-claude-code-vs-copilot-ai-ide",
...
##### 41. data-engineering-azure-pipelines-lakehouse

```json
{
  "url": "data-engineering-azure-pipelines-lakehouse",
...
#### Batch C — Articles 42–51 (Format B)

##### 42. data-engineering-batch-vs-streaming

```json
{
  "url": "data-engineering-batch-vs-streaming",
...
```json
{
  "url": "database-indexing-strategies",
...
##### 44. database-optimization-entity-framework

```json
{
  "url": "database-optimization-entity-framework",
...
##### 45. database-transactions-isolation-levels

```json
{
  "url": "database-transactions-isolation-levels",
...
##### 46. dependency-injection-dotnet-core

```json
{
  "url": "dependency-injection-dotnet-core",
...
##### 47. design-patterns-overview-creational-structural-behavioral

```json
{
  "url": "design-patterns-overview-creational-structural-behavioral",
...
##### 48. developers-integrating-ai-daily-workflows

```json
{
  "url": "developers-integrating-ai-daily-workflows",
...
##### 49. domain-driven-design-basics

```json
{
  "url": "domain-driven-design-basics",
...
##### 50. dotnet-core-middleware-pipeline

```json
{
  "url": "dotnet-core-middleware-pipeline",
...
##### 51. event-driven-architecture-azure

```json
{
  "url": "event-driven-architecture-azure",
...
#### Batch D — Articles 52–61 (Format B)

##### 52. event-sourcing-and-cqrs
...
##### 53. feature-flags-toggles-dotnet

```json
{
  "url": "feature-flags-toggles-dotnet",
...
##### 54. full-stack-net-angular-enterprise

```json
{
  "url": "full-stack-net-angular-enterprise",
...
##### 55. gamification-enterprise-apps

```json
{
  "url": "gamification-enterprise-apps",
...
##### 56. grpc-vs-rest-dotnet-apis

```json
{
  "url": "grpc-vs-rest-dotnet-apis",
...
##### 57. impact-ai-tools-code-quality-maintainability

```json
{
  "url": "impact-ai-tools-code-quality-maintainability",
...
##### 58. kubernetes-basics-dotnet-developers

```json
{
  "url": "kubernetes-basics-dotnet-developers",
...
##### 59. microservices-resilience-circuit-breaker-retry

```json
{
  "url": "microservices-resilience-circuit-breaker-retry",
...
##### 60. mobile-app-architecture-vue-capacitor

```json
{
  "url": "mobile-app-architecture-vue-capacitor",
...
#### Batch E — Articles 62–71 (Format B)

##### 62. oauth2-openid-connect-dotnet

```json
{
  "url": "oauth2-openid-connect-dotnet",
...
##### 63. observability-dotnet-azure

```json
{
  "url": "observability-dotnet-azure",
...
##### 64. open-telemetry-distributed-tracing-dotnet

```json
{
  "url": "open-telemetry-distributed-tracing-dotnet",
...
##### 65. owasp-api-security-top-10

```json
{
  "url": "owasp-api-security-top-10",
...
##### 66. property-management-systems-uk

```json
{
  "url": "property-management-systems-uk",
...
##### 67. repository-pattern-unit-of-work-dotnet

```json
{
  "url": "repository-pattern-unit-of-work-dotnet",
...
##### 68. rest-api-versioning-idempotency

```json
{
  "url": "rest-api-versioning-idempotency",
...
##### 69. rest-vs-graphql-apis

```json
{
  "url": "rest-vs-graphql-apis",
...
##### 70. saga-pattern-orchestrator-vs-choreography

```json
{
  "url": "saga-pattern-orchestrator-vs-choreography",
...
##### 71. securing-apis-dotnet

```json
{
  "url": "securing-apis-dotnet",
...
#### Batch F — Articles 72–82 (Format B)

##### 72. solid-principles-in-practice

```json
{
  "url": "solid-principles-in-practice",
...
##### 73. sql-server-performance-tuning

```json
{
  "url": "sql-server-performance-tuning",
...
##### 74. structural-design-patterns-dotnet

```json
{
  "url": "structural-design-patterns-dotnet",
...
##### 75. technical-leadership-remote-teams

```json
{
  "url": "technical-leadership-remote-teams",
...
##### 76. testing-strategies-unit-integration-e2e

```json
{
  "url": "testing-strategies-unit-integration-e2e",
...
##### 77. trade-offs-ai-code-generation

```json
{
  "url": "trade-offs-ai-code-generation",
...
##### 78. vue-enterprise-scale

```json
{
  "url": "vue-enterprise-scale",
...
##### 79. vue-vs-angular-vs-react-full-comparison

```json
{
  "url": "vue-vs-angular-vs-react-full-comparison",
...
##### 80. what-developers-want-from-ai-assistants

```json
{
  "url": "what-developers-want-from-ai-assistants",
...
##### 81. where-ai-fails-real-world-software-development

```json
{
  "url": "where-ai-fails-real-world-software-development",
...
##### 82. why-ai-productivity-gains-plateau

```json
{
  "url": "why-ai-productivity-gains-plateau",
...
- **Total URLs:** 81 (4 core + 7 services + 10 projects + 56 blog + 404/fallback).
- **Strategy:** Content-driven only. No fixed or generic keyword sets.
- **Heat Exchanger:** The keyword examples you gave (e.g. long-tail primary) were for **shape only**. The Heat Exchanger page content is about **petroleum, oil & gas, heat exchanger monitoring, predictive maintenance, refineries, OpenShift, Grafana, Prometheus** — it has **nothing to do with financial services or financial analytics**. Keywords for that URL were generated from the actual project overview, architecture, and tech stack (thermal management, equipment failure forecasting, 2.5M data points daily, etc.).
- **Next steps:** Use this doc as source of truth; implement primary/secondary/semantic in code (meta, schema, or LLM manifest) when ready; re-run generation from page content after any major copy changes.

**Re-read and revision done:** Strategy is content-driven only; all 81 URLs have keywords derived from page content; Heat Exchanger is explicitly petroleum/oil & gas, not financial. After you confirm, the next pass can refine any row and then implementation can follow.
