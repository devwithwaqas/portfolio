#!/usr/bin/env python3
"""
Final Smart City C4 Diagram Component Extraction
Uses area logic to eliminate labels and properly identify all components
"""
...
def extract_smartcity_components_final(svg_file: str) -> Dict[str, Any]:
    """Final extraction using area logic to eliminate labels"""
    ...
    for group in all_groups:
        ...
        for rect in rectangles:
            ...
            # Use area logic: exclude very small shapes (labels) and very large shapes (backgrounds)
            if 2000 < area < 200000:  # Reasonable component size range
        ...
        for ellipse in ellipses:
            ...
            if 2000 < area < 200000:  # Same area logic
                rect_key = f"{x}_{y}_{width}_{height}"
                if rect_key not in processed_ellipses:
                    valid_rects.append({
                        'x': x, 'y': y, 'width': width, 'height': height,
                        'area': area, 'rect': ellipse, 'type': 'ellipse'
                    })
                    processed_ellipses.add(rect_key)
...
def main():
    ...
    print("Final extraction of Smart City C4 diagram components using area logic...")
    components = extract_smartcity_components_final(svg_file)
    ...
    # Save to JSON
    output_file = "smartcity_final_components.json"
    ...
    for i, shape in enumerate(components['individual_shapes'][:5]):
        print(f"  {i+1}. {shape['text']} ({shape['type']}) - {shape['width']:.1f}x{shape['height']:.1f} (area: {shape['area']:.0f})")
    ...
    for i, user in enumerate(components['users']):
        print(f"  {i+1}. {user['text']} - {user['width']:.1f}x{user['height']:.1f} (area: {user['area']:.0f})")
