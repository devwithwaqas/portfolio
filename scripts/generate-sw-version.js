...
function updateServiceWorkerVersion() {
  ...
  if (!fs.existsSync(swPath)) {
    console.warn('âš ï¸  sw.js not found, skipping version update')
    return version
  }
...
function updateMainJsVersion(version) {
  ...
  if (!fs.existsSync(mainJsPath)) {
    console.warn('âš ï¸  main.js not found, skipping version update')
    return
  }
  ...
  console.log(`âœ… Updated main.js expected version to: ${version}`)
}

if (require.main === module) {
  const version = updateServiceWorkerVersion()
  updateMainJsVersion(version)
  console.log(`\nðŸŽ‰ Service Worker versioning complete: ${version}`)
}
