...
router.beforeEach((to, from, next) => {
  ...
  // Set SEO based on route (clean apply* API; setPageSEO for edge cases)
  if (to.name === 'Home') {
    applyHomeSEO()
    ...
    const serviceInfo = SERVICE_DATA_MAP[to.name] || {
      title: to.name.replace(/([A-Z])/g, ' $1').trim(),
      description: `Expert ${to.name.replace(/([A-Z])/g, ' $1').trim()} services. Enterprise solutions, Azure Cloud, and .NET development.`
    }
    const serviceData = {
    ...
    setPageSEO({
      ...
      keywords: ['Senior Software Engineer', 'Technical Lead'],
      url: `${SITE_URL}${to.path}`
    })
  }
