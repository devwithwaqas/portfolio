...
function calculateSimilarity(str1, str2) {
  ...
  const baseScore = (matches / maxWords) * 100
  
  // Bonus for substring matches
  let substringBonus = 0
  const longer = words1.length > words2.length ? s1 : s2
  const shorter = words1.length > words2.length ? s2 : s1
  
  if (longer.includes(shorter)) {
    substringBonus = 20
  }
  
  return Math.min(100, Math.round(baseScore + substringBonus))
}
...
function findBestLocalIcon(techName) {
  ...
  LOCAL_ICONS.forEach(iconFile => {
    // Remove file extension and -optimized suffix for comparison
    // This handles files that might be stored as "filename-optimized.png"
    let iconName = iconFile.replace(/\.(svg|png|jpg|jpeg|webp|avif)$/i, '')
    iconName = iconName.replace(/-optimized$/i, '') // Strip -optimized suffix if present
    
    const score = calculateSimilarity(normalized, iconName)
    
    if (score > bestScore && score >= 50) { // Minimum 50% match
      bestScore = score
      // Return the base filename (without -optimized) so OptimizedImage can add it back
      // Extract original extension
      const extMatch = iconFile.match(/\.(svg|png|jpg|jpeg)$/i)
      const ext = extMatch ? extMatch[1] : 'png'
      bestMatch = `${iconName}.${ext}`
    }
  })
...
export function resolveIcon(techName, fallbackName = null) {
  ...
  if (exactMatch) {
    ...
    if (exactMatch.type === 'local') {
      ...
      if (localIcon) {
        ...
        if (localIcon.toLowerCase().endsWith('.svg')) {
          return {
            type: 'local',
        ...
        // For PNG/JPG files, return base path - OptimizedImage will auto-detect optimized versions
        // OptimizedImage handles adding -optimized suffix and finding WebP/AVIF versions
        return {
          type: 'local',
          src: assetPath(`/assets/img/Icons/${localIcon}`),
          alt: techName
    ...
    if (exactMatch.type === 'devicon') {
      ...
      if (exactMatch.local) {
        const localIcon = exactMatch.local
        // For SVG files, return as-is (no optimization needed)
        if (localIcon.toLowerCase().endsWith('.svg')) {
          return {
            type: 'local',
        ...
        // For PNG/JPG, return base path - OptimizedImage handles optimization
        return {
          type: 'local',
          src: assetPath(`/assets/img/Icons/${localIcon}`),
          alt: techName
  ...
  // 4. Try fuzzy matching with local icons
  const bestLocalMatch = findBestLocalIcon(techName)
  if (bestLocalMatch) {
    // findBestLocalIcon already returns base filename (without -optimized)
    // Extract extension
    const extMatch = bestLocalMatch.match(/\.(svg|png|jpg|jpeg)$/i)
    const ext = extMatch ? extMatch[1].toLowerCase() : 'png'
    
    // For SVG files, return as-is (no optimization)
    if (ext === 'svg') {
      return {
        type: 'local',
    ...
    // For PNG/JPG files, return base path - OptimizedImage will add -optimized suffix
    return {
      type: 'local',
      src: assetPath(`/assets/img/Icons/${bestLocalMatch}`),
      alt: techName
